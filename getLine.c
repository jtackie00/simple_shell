 ...
 * Return: bytes read
 */
ssize_t input_buf(info_a *inf, char **buff, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buff = NULL;
		...
		if (r > 0)
		{
			...
			build_history_list(inf, *buff, inf->hcount++);
 ...
 * Return: bytes read
 */
ssize_t get_input(info_a *inf)
{
	static char *buff; /* the ';' command chain buffer */
	static size_t i, j, len;
	ssize_t r = 0;
	char **buf_p = &(inf->argu), *p;
 ...
 * Return: r
 */
ssize_t read_buf(info_a *inf, char *buff, size_t *i)
{
	ssize_t r = 0;
 ...
 * Return: s
 */
int _getline(info_a *inf, char **ptr, size_t *length)
{
