 ...
 * Return: size of list
 */
list_a *add_node(list_a **head, const char *str, int num)
{
	list_a *new_head;

	if (!head)
		return (NULL);
	new_head = malloc(sizeof(list_a));
	if (!new_head)
		return (NULL);
	_memset((void *)new_head, 0, sizeof(list_a));
	new_head->num = num;
	if (str)
	{
...
/**
 * print_list_str - prints only the str element of a list_a linked list
 * @h: pointer to first node
 *
 * Return: size of list
 */
size_t print_list_str(const list_a *h)
{
	size_t i = 0;
